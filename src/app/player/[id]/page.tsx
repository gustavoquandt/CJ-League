...
const ActiveRatingDot = (props: any) => {
  const { cx, cy, payload } = props;
  if (!cx || !cy) return null;
  const handleClick = () => {
    if (payload.matchId) {
      window.open(`https://www.faceit.com/en/cs2/room/${payload.matchId}`, '_blank');
    }
  };
  return (
    <circle cx={cx} cy={cy} r={6}
      ...
      style={{ cursor: payload.matchId ? 'pointer' : 'default' }}
      onClick={handleClick}
    />
  );
};
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={ratingChartData} margin={{ top: 8, right: 4, left: -20, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="rating" stroke="#0EA5E9" strokeWidth={2}
                      fill="url(#ratingGrad)" dot={<RatingDot />}
                      activeDot={<ActiveRatingDot />} />
                  </AreaChart>
                </ResponsiveContainer>
            ...
            <Card>
              ...
              {comparePlayer && (() => {
                const rows = [
                  ...
                  { label: 'Flashes/Jogo', a: player.matchesPlayed > 0 ? (player.totalFlashSuccesses || 0) / player.matchesPlayed : 0, b: comparePlayer.matchesPlayed > 0 ? (comparePlayer.totalFlashSuccesses || 0) / comparePlayer.matchesPlayed : 0, fmt: (v: number) => v.toFixed(1) },
                ];
                return (
                  <div>
